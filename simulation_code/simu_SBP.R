...
for(i in 1:nrow(paras)){
    ...
    out1 = mclapply(1:1000,function(x){
        ...
        sige = 1 - var(beta*sim_bed + sim_cov1 %*% cov1_eff + sim_cov2 %*% (gamma * cov2_eff))
        sim_y = beta*sim_bed + sim_cov1 %*% cov1_eff + sim_cov2 %*% (gamma * cov2_eff) + rnorm(n,0,sqrt(sige)) 
        m1 = summary(lm(sim_y~sim_cov1+sim_cov2+sim_bed))
        m1.5 = summary(lm(sim_y~sim_cov1 + sim_cov2[,1:5]+sim_bed))
        m1.10 = summary(lm(sim_y~sim_cov1 + sim_cov2[,1:10]+sim_bed))
        m0 = summary(lm(sim_y~sim_cov1+sim_bed))
